...
const double trace(const Matrix& m) {
	auto minsize = (int)fmin(m.rows(), m.cols());
	double ret = 0.0;
...
int rang(const Matrix& m) {
	...
	Matrix s, u, vt;
	SVD(m, s, u, vt);
	auto tol = fmax(m.rows(), m.cols())*max(s)*MATRIX_ZERO; // method get from Matlab rank function
